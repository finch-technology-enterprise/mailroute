...
export default function Vendors() {
  ...
  const filtered = vendors.filter((v) =>
    !search || v.name.toLowerCase().includes(search.toLowerCase()) || v.fromEmail.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handlePrioritySave = async (vendor: Vendor) => {
    ...
    try {
      ...
      setVendors((prev) => prev.map((v) => v.id === vendor.id ? updated.data : v));
      setEditingPriority(null);
      toast("Priority updated", "success");
    } catch (err) {
      toast(err instanceof Error ? err.message : "Failed to update priority", "error");
    }
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ type: "spring", bounce: 0, duration: 0.35 }}>
      <div className="mb-6 flex items-center justify-between" style={{ flexWrap: "wrap", gap: 12 }}>
        ...
        <div className="flex gap-3">
          <input className="apple-input" style={{ width: 200, fontSize: 14, padding: "8px 12px", minHeight: 36 }} placeholder="Search vendors…" value={search} onChange={(e) => setSearch(e.target.value)} />
          <motion.button className="apple-btn apple-btn-primary" onClick={() => { setEditingVendor(null); setFormOpen(true); }} whileTap={{ scale: 0.97 }}>Add Vendor</motion.button>
        </div>
      </div>
      ...
      {!loading && filtered.length === 0 ? (
        <EmptyState title={search ? "No matching vendors" : "No vendors configured"} description={search ? "Try a different search term." : "Add your first email vendor to start sending."} actionLabel={search ? undefined : "Add Vendor"} onAction={search ? undefined : () => { setEditingVendor(null); setFormOpen(true); }} />
      ) : (
        <Table
          columns={[
            ...
            {
              key: "priority", header: "Priority", render: (v: Vendor) =>
                editingPriority === v.id ? (
                  <div className="flex gap-1" style={{ alignItems: "center" }}>
                    <input type="number" min={1} max={999} className="apple-input" style={{ width: 64, fontSize: 13, padding: "4px 8px", minHeight: 28 }} value={priorityVal} onChange={(e) => setPriorityVal(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handlePrioritySave(v)} autoFocus />
                    <button className="apple-link" onClick={() => handlePrioritySave(v)} style={{ fontSize: 11 }}>Save</button>
                  </div>
                ) : (
                  <button className="apple-link" style={{ fontSize: 13, fontWeight: 500, color: "var(--text-secondary)" }} onClick={() => { setEditingPriority(v.id); setPriorityVal(String(v.priority)); }}>{v.priority}</button>
                ),
            },
